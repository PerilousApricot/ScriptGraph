...
g = Graph.Graph()
baseWorkDir = "/uscms_data/d2/meloam/vandy-submit-johntest"
...
for dataset in qcd_datasets:
    treeNode,produceTree = genTree( g=g,input = nullInput,
                                ...
                                crabCfg  = "/uscms_data/d2/meloam/vandy-submit2/crab_mc.cfg", 
                                cmsswCfg = "/uscms_data/d2/meloam/vandy-submit2/cmssw_mc.py",
                                dataset  = dataset[0],
                                crabReplacements  = [ 
                                      [ "REMOTEDIR", "user_remote_dir=39x23/%s" %
                                        (dataset[1],) ]
                                    ] 
                                )
...
#                                crabReplacements  = [ [ "RUNSELECTION", "" ] ] )
        ...
        for trigger in trigger_list_linked[dataset[1]]:
            ...
            treeNode,produceTree = genTree( g = g, input = nullInput,
                                ...
                                crabCfg  = "/uscms_data/d2/meloam/vandy-submit2/crab_data.cfg", 
                                cmsswCfg = "/uscms_data/d2/meloam/vandy-submit2/cmssw_data.py",
                                dataset  = dataset[0],  
                                cmsswReplacements = [ ["CURRENTTRIGGER", trigger[0] ] ],
                                crabReplacements  = [ [ "RUNSELECTION", "lumi_mask=%s" %
                                                                    (trigger[5]) ],
                                                      [ "REMOTEDIR", "user_remote_dir=39x2/%s-%s" %
                                                                (dataset[1],trigger[1]) ]] )
