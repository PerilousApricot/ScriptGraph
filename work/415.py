...
baseWorkDir = "/uscms_data/d2/meloam/input415x4"
...
operating_points = ["SSVHEM", "TCHEM","SSVHPT",]
        ...
        for bin in jet_bins:
            for tmap in dataTriggerMiter.iterGrouped( 'bin' ):
                ...
                for onetmap in tmap.iterMany():
                    sample = onetmap[0][5]
                    trigger = onetmap[0]
                    step_postfix = "-%s-%s-%s-%s-%s%s" % (sample,trigger[1],bin[0],opoint,slice[2])
                    monitor_node =\
...
for slice in slice_bins:
    for opoint in operating_points:
        for bin in jet_bins:  
            currNode = hadd_helper( g, "-%s-%s-data%s" % (bin[0],opoint,slice[2]), 
                                            skiplessDataMiter.getValues( bin=bin[0],
                                            opoint = opoint, slice=slice) )
            haddedDataMiter.add(
                                currNode,
                                opoint = opoint,
                                bin = bin[0], slice=slice )
            addSingleMonitor( currNode,\
                                                    ...
                                                    slice   = slice,
                                                    type    = "data_binned" )
            dataForWeights[ bin[0] ][ slice[0] ] = currNode
            raise "change this guy above to handle slice"
            if opoint == operating_points[0]:
                dataForPlots.add( currNode, bin = bin[0], slice=slice )
...
for slice in slice_bins:
    for opoint in haddedDataMiter.iterGrouped( 'opoint' ):
        currNode = hadd_helper( g, "-%s%s-data-final" % (opoint.vals[0][1]['opoint'],slice=slice), 
                                    skiplessDataMiter.getValues( opoint = opoint.vals[0][1]['opoint']), slice=slice )
        haddedDataFinalMiter.add(
                            currNode,
                            opoint = opoint.vals[0][1]['opoint'], slice=slice)

        addSingleMonitor( currNode,\
                                                opoint  = opoint.vals[0][1]['opoint'],
                                                syst    = "nominal",
                                                type    = "data_final" slice=slice )
        if opoint.vals[0][1]['opoint'] == operating_points[0]:
            dataForPlots.add( currNode, bin = "all", slice=slice )
#
...
for systematic in systematicDataList:   
    for opoint in haddedDataMiter.iterGrouped( 'opoint' ):
        ...
        haddedDataFinalSystMiter.add( currNode, opoint  = opoint.vals[0][1]['opoint'],
                                                    syst    = systematic[0] )
...
for currmerge in preweightMiter.iterGrouped( 'bin' ):
    ...
    addSingleMonitor( mergeNode,\
                                    bin     = currmerge.vals[0][1]['bin'],
                                    syst    = "nominal",
                                    type    = "mc_unweight_binned" )
...
for currmerge in reweightMiter.iterGrouped( 'bin', 'opoint' ):
    ...
    addSingleMonitor( mergeNode, \
                                                 ...
                                                 opoint  = currmerge.vals[0][1]['opoint'],
                                                 syst = "nominal",
                                                 type = "mc_reweight_pthat" )


    if currmerge.vals[0][1]['opoint'] == operating_points[0]:
        mcForPlots.add( mergeNode, bin = currmerge.vals[0][1]['bin'] )
...
for opoint in reweightMergePthatMiter.iterGrouped( 'opoint' ):
    ...
    addSingleMonitor( monitor_node, \
                                                 opoint  = opoint.vals[0][1]['opoint'],
                                                 syst = "nominal",
                                                 type = "mc_reweight_final" )
    if opoint.vals[0][1]['opoint'] == operating_points[0]:
        mcForPlots.add( currNode, bin = "all" )
...
webRoot  = "/afs/fnal.gov/files/home/room3/meloam/public_html/s8/415x4/"
#webRoot  = "/uscms_data/d2/meloam/input39x2/edges/plots/"
httpRoot = "http://home.fnal.gov/~meloam/s8/415x4/"
...
allDoublePlots = []
print mcForPlots.vals
print dataForPlots.vals
