...
g = Graph.Graph()
baseWorkDir = "/uscms_data/d2/meloam/vandy-submit"
...
#                                crabReplacements  = [ [ "RUNSELECTION", "" ] ] )
        ...
        for trigger in trigger_list_linked[dataset[1]]:
            ...
            treeNode,produceTree = genTree( g = g, input = nullInput,
                                ...
                                crabCfg  = "/uscms_data/d2/meloam/vandy-submit/crab_data.cfg", 
                                cmsswCfg = "/uscms_data/d2/meloam/vandy-submit/cmssw_data.py",
                                dataset  = dataset[0],  
                                cmsswReplacements = [ ["CURRENTTRIGGER", trigger[0] ] ],
                                crabReplacements  = [ [ "RUNSELECTION", "lumi_mask=%s" %
                                                                    (trigger[5]) ],
                                                      [ "REMOTEDIR", "user_remote_dir=39x/%s-%s" %
                                                                (dataset[1],trigger[1]) ]] )
